...
import { resolveFood } from '../_shared/grounding/cascade.ts';
...
/**
 * analyze-food — the phase tracer path (kind=photo). Gate order is fixed by
 * the RESEARCH architecture diagram: preflight → request parse → auth →
 * entitlement/quota → kind routing → VLM → grounding → arithmetic → persist →
 * respond. 500 INTERNAL is reserved for genuine bugs; malformed VLM output is
 * a typed 422 and malformed requests are typed 400s.
 */
...
export async function handler(req: Request): Promise<Response> {
  try {
    ...
    if (request.kind !== 'photo') {
      return errorJson(400, 'VALIDATION_ERROR', [
        `kind '${request.kind}' is not implemented yet`,
      ]);
    }
    ...
    try {
      vlmParsed = VlmOutputSchema.parse(
        await provider.analyze({ kind: request.kind, imageBase64: request.imageBase64 }, 'tier1'),
      );
    } catch (error) {
